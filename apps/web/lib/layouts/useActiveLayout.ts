 ...
 *
 * Reads `getTheme(theme.activeTheme).layouts?.[page]`. Returns undefined when
 * the active theme has no layout for that page, in which case the page should
 * render its built-in layout (unchanged behaviour). This is the single hook
 * every storefront page uses to opt into Theme Studio layouts, keeping the
