...
export default function CategoryPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const catRes = await fetch(`${API_URL}/categories/${encodeURIComponent(slug)}`);
  ...
  // A category that does not exist must be a real 404, not an empty page
  // returning HTTP 200 (a "soft 404" that search engines penalise).
  if (missing) notFound();
