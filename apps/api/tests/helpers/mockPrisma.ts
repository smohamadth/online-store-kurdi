...
function match(row: Row, where: any): boolean {
  ...
  for (const [k, v] of Object.entries(where)) {
    ...
    if (v === null) {
      ...
      // operator object?
      const operators = ['contains', 'gte', 'lte', 'gt', 'lt', 'not', 'startsWith', 'endsWith', 'mode'];
      ...
      if (isOperatorObj) {
        ...
        for (const [subK, subV] of Object.entries(v)) {
          ...
          if (subK === 'slug' && k === 'category') {
            // join through categoryId -> Category.slug
            const catStore = storeFor('Category');
            const cat = catStore.get(row.categoryId);
            if (!cat || cat.slug !== subV) return false;
          } else {
